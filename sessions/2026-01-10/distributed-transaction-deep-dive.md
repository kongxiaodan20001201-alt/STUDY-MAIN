...
sequenceDiagram
    ...
    participant RM_Order as 订单库代理(RM)
    participant RM_Stock as 库存服务(RM)
    participant RM_Points as 积分服务(RM)

    Note over TM, RM_Order: (通常在同一个微服务进程中)

    %% --- Phase 1: 业务执行与加锁 ---
    User->>TM: 提交下单请求
    
    rect rgb(240, 248, 255)
        Note right of TM: 0. 开启全局事务
        TM->>TC: GlobalBegin (申请 XID)
        TC-->>TM: 返回 XID (tx_123)
    end

    rect rgb(255, 250, 240)
        Note right of TM: 1. 执行订单业务
        TM->>RM_Order: 创建订单 (Local Transaction)
        RM_Order->>RM_Order: 解析 SQL & 记录 UndoLog
        RM_Order->>TC: registerBranch (申请全局锁: order:1)
        TC-->>RM_Order: 锁获取成功
        RM_Order->>RM_Order: 提交本地事务 (释放 DB 连接)
        RM_Order-->>TM: 执行成功
    end

    rect rgb(255, 250, 240)
        Note right of TM: 2. 远程调用库存 (传递 XID)
        TM->>RM_Stock: 扣减库存 (Header: XID)
        RM_Stock->>RM_Stock: 解析 SQL & 记录 UndoLog
        RM_Stock->>TC: registerBranch (申请全局锁: stock:1)
        TC-->>RM_Stock: 锁获取成功
        RM_Stock->>RM_Stock: 提交本地事务 (释放 DB 连接)
        RM_Stock-->>TM: 执行成功
    end

    rect rgb(255, 250, 240)
        Note right of TM: 3. 远程调用积分 (传递 XID)
        TM->>RM_Points: 扣减积分 (Header: XID)
        RM_Points->>RM_Points: 解析 SQL & 记录 UndoLog
        RM_Points->>TC: registerBranch (申请全局锁: points:88)
        TC-->>RM_Points: 锁获取成功
        RM_Points->>RM_Points: 提交本地事务 (释放 DB 连接)
        RM_Points-->>TM: 执行成功
    end
